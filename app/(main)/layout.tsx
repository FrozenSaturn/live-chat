...
export default function MainLayout({
    ...
    return (
        <div className="flex h-screen bg-zinc-50 font-sans dark:bg-black overflow-hidden relative">
            ...
            <div className={`${isChatPage ? "hidden md:flex md:w-80" : "flex w-full md:w-80"} h-full shrink-0`}>
                <Sidebar className="w-full" />
            </div>

            {/* Main Content Area: Show on desktop always, on mobile only if on a chat page */}
            <div className={`${isChatPage ? "flex" : "hidden md:flex"} flex-1 flex-col overflow-hidden min-w-0`}>
                {children}
            </div>
        </div>
    );
